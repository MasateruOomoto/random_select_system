...
public class AuthAction extends ActionBase {
    ...
    public void login() throws ServletException, IOException {
        ...
        String userId = getRequestParam(AttributeConst.USER_USER_ID);
        String plainPass = getRequestParam(AttributeConst.USER_PASS);
        String pepper = getContextScope(PropertyConst.PEPPER);

        //有効なユーザーか認証する
        Boolean isValidUser = service.validateLogin(userId, plainPass, pepper);
        ...
        if (isValidUser) {
            ...
            if (checkToken()) {
                ...
                //ログインしたユーザーのDBデータを取得
                UserView uv = service.findOne(userId, plainPass, pepper);
                //セッションにログインしたユーザーを設定
            ...
            //入力された従業員コードを設定
            putRequestScope(AttributeConst.USER_USER_ID, userId);
