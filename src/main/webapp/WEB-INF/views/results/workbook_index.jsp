            ...
            <tbody>
                ...
                <c:forEach var="workbook" items="${workbooks}" varStatus="status">
                    <tr class="row${status.count % 2}">
                        ...
                        <td>
                            <a href="<c:url value='?action=${actCha}&command=${commIdxStu}&workbook_id=${workbook.id}' />"><c:out value="${workbook.workbookName}" /></a>
                        </td>
                    </tr>
                </c:forEach>
            </tbody>
        ...
        <div id="pagination">
            ...
            <c:forEach var="i" begin="1" end="${((workbooks_count - 1) / maxRow) + 1}" step="1">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="<c:url value='?action=${actWor}&command=${commIdxStu}&page=${i}' />"><c:out value="${i}" /></a>&nbsp;
                    </c:otherwise>
                </c:choose>
            </c:forEach>
        </div>
